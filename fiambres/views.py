...
def listado_productos_sinprecio(request):
        busqueda = request.GET.get("buscar")
        #usuarios = Usuario.objects.filter(tipo_usuario = cat)
        #categoria_precio = Precios.objects.filter(tipo_usuario = request.tipo_usuario)
        ...
        if busqueda:
            ...
            return render(request, 'fiambres/listado_productos_sinprecio.html', {'products':products, 'categories': categories})
        else:
            return render(request, 'fiambres/listado_productos_sinprecio.html', {'products': products, 'categories': categories})
...
def buscar_categoriass_sinprecio(request,id):
    busqueda = request.GET.get("buscar")
    template_name='fiambres/listado_productos_sinprecio.html'
    cat=Category.objects.get(id=id)
    categories_sinprecio = Category.objects.all()
    products = Product.objects.filter(available=True, category=cat)
    context = {"products":products, "categories_sinprecio":categories_sinprecio}
    ...
    if busqueda:
            ...
            return render(request, 'fiambres/listado_productos_sinprecio.html', {'products':products, 'categories_sinprecio': categories_sinprecio})
    else:
            return render(request, 'fiambres/listado_productos_sinprecio.html', context)
...
class Verfactura(ValidarPermisosRequeridosMixin,UpdateView):
    model = Order
    template_name = 'fiambres/factura_id.html'
    permission_required = ('fiambres.view_factura_id','fiambres.add_factura_id','fiambres.delete_factura_id','fiambres.change_factura_id')
    context_object_name = 'factura_id'
...
def verfactura(request,id):
    if Order.objects.filter(id=id).exists():
        template_name = 'fiambres/factura_id.html'
        factura_id = Order.objects.filter(id=id)
        context = {"factura_id":factura_id,}
        return render(request, template_name, context)
